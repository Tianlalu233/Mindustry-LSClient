...
//                                    .addListener(new Tooltip(l -> l.label(() -> effect.localizedName + " [lightgray]" + UI.formatTime(unit.getDuration(effect))).style(Styles.outlineLabel)));
    ...
    public void drawItems(Unit unit){
        ...
        if(unit.item() != null && unit.itemTime > 0.01f){
            ...
            if((Core.settings.getBool("showunititemsamount") || unit.isLocal()) && !renderer.pixelator.enabled()){
                Fonts.outline.draw(unit.stack.amount + "",
                unit.x + Angles.trnsx(unit.rotation + 180f, itemOffsetY),
                unit.y + Angles.trnsy(unit.rotation + 180f, itemOffsetY) - 3,
                color, 0.25f * unit.itemTime / Scl.scl(1f), false, Align.center
                );
            }
    ...
    public void drawEngine(Unit unit){
        ...
        Draw.color(engineColor == null ? unit.team.color : engineColor, alpha);
        Fill.circle(
        ...
        Draw.color(engineColorInner, alpha);
        Fill.circle(
