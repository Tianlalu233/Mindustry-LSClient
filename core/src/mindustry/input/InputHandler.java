...
public abstract class InputHandler implements InputProcessor, GestureListener{
    ...
    protected void removeSelection(int x1, int y1, int x2, int y2, boolean flush, int maxLength){
        ...
        removed.addAll(player.team().data().removeBrokenBlocks());
    ...
    boolean tileTapped(@Nullable Building build){
        ...
        boolean showConfig = settings.getBool("showenemyconfig") || build.interactable(player.team());
        //check if tapped block is configurable
        if(build.block.configurable && showConfig){
            consumed = true;
